...
- content rank と missing chunks を用いた request-level B2 decision；
...
- `zipf_alpha x neighbor_cache_rank_gamma` の sensitivity analysis；
...
Phase 1.1 では，B2 は scenario 単位ではなく request 単位で判定します。local recovery の後に不足している chunk 数，content rank，neighbor availability，および rank-aware cache probability を用いて，近隣復元成功確率を次のように近似します。
...
ここでの `P_success` は第一段階の簡略化された確率モデルであり，online trust learning によって推定された値ではありません。このモデルの目的は，hot content では neighbor search を選びやすくし，cold content や成功確率が低い条件では無効な neighbor probing を抑制する B2 の基本挙動を確認することです。
...
| `neighbor_cache_hot_prob` | `0.90` | 非常に hot な content が neighbor ES に cache されている推定確率。 |
| `neighbor_cache_cold_prob` | `0.15` | cold content に対する neighbor cache probability の下限。 |
| `neighbor_cache_rank_gamma` | `1.0` | content rank に伴う cache probability 低下の強さ。 |
...
### `scripts/run_b2_zipf_sweep.py`

Phase 1.1 で追加した B2/Zipf sensitivity analysis の entry point です。`neighbor_es_availability x origin_delay` heatmap を維持しつつ，次の grid を追加します。
...
さらに hot / mid / cold content rank bucket ごとの B2 neighbor choice rate を出力します。これにより，B2 が hot request と cold request に対して異なる判断をしているかを直接確認できます。
...
| `local_failure_rate` | local ES group だけでは復元できなかった request の割合。 |
| `neighbor_attempt_rate` | 全 request のうち neighbor search を試した割合。 |
| `neighbor_failure_rate` | neighbor fallback を試した後に失敗し，origin に戻った割合。 |
| `b2_neighbor_choice_rate` | B2 のみ: local failure 後に neighbor search を選択した割合。 |
...
| `results/phase1_b2_zipf/` | Phase 1.1 の result batch。request-level B2 と Zipf/cache-rank sensitivity をまとめる。 |
| `results/phase1_b2_zipf/manifest.json` | 実行 command，git state，model defaults，output file list を記録する manifest。 |
| `docs/project_map.md` | project navigation。各 directory の役割と今後の result batch rule を説明する。 |
| `docs/experiment_guide.md` | Phase 1.1 reproduction guide。command，outputs，metrics の意味を説明する。 |
...
Phase 1.1 request-level B2/Zipf formal run:
...
- Zipf request probability が `zipf_alpha` の増加に伴って hot content に偏ること；
- neighbor cache probability が content rank に対して単調に低下すること；
- neighbor recovery が固定の `K` ではなく `missing_chunks` を使うこと；
- 同一 scenario 内で B2 が hot request と cold request に異なる判断を行えること；
- `B2` の expected delay が neighbor availability と cache probability を使うこと；
