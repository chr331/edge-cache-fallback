...
- 将 `B2` 改成 request-level 决策，并使用 content rank 与 missing chunks；
...
- 增加 `zipf_alpha x neighbor_cache_rank_gamma` 敏感性分析；
...
Phase 1.1 之后，`B2` 不再做“一个场景只判断一次”的固定选择，而是在每个 request 上根据 local 缺多少 chunk、请求内容的 rank、neighbor 可用率和缓存概率来判断：
...
注意：这里的 `P_success` 仍然只是第一阶段的简化概率模型，不是在线学习出来的 trust score。它的作用是让 `B2` 能体现“热门内容更值得问 neighbor，冷门内容或低成功率条件下应该少问 neighbor”的基本逻辑。
...
| `neighbor_cache_hot_prob` | `0.90` | 极热门内容在 neighbor ES 中被缓存的估计概率。 |
| `neighbor_cache_cold_prob` | `0.15` | 冷门内容 neighbor 缓存概率的下限。 |
| `neighbor_cache_rank_gamma` | `1.0` | 控制缓存概率随 content rank 下降的强度。 |
...
这是 Phase 1.1 新增的 B2/Zipf 敏感性分析入口。它继续保留 `neighbor_es_availability x origin_delay` heatmap，同时新增：
...
它还会输出 hot / mid / cold content rank bucket 下 `B2` 选择 neighbor 的比例，用来直接检查 `B2` 是否真的对热门和冷门请求做出了不同选择。
...
| `local_failure_rate` | local ES 无法完成恢复的请求比例。 |
| `neighbor_attempt_rate` | 所有请求中尝试 neighbor search 的比例。 |
| `neighbor_failure_rate` | 尝试 neighbor fallback 后仍然失败、最后回 origin 的比例。 |
| `b2_neighbor_choice_rate` | 仅对 B2：local 失败后选择 neighbor search 的比例。 |
...
| `results/phase1_b2_zipf/` | Phase 1.1 的新结果批次，集中存放 request-level B2 与 Zipf/cache-rank 敏感性分析。 |
| `results/phase1_b2_zipf/manifest.json` | 本批次命令、git 状态、模型默认参数和输出文件清单。 |
| `docs/project_map.md` | 项目导航，说明每类文件是什么、后续新结果应该放在哪里。 |
| `docs/experiment_guide.md` | Phase 1.1 复现实验指南，写清楚命令、输出和指标含义。 |
...
运行 Phase 1.1 request-level B2/Zipf 正式实验：
...
- Zipf 请求概率是否随 `zipf_alpha` 增大而更偏向热门内容；
- neighbor cache probability 是否随 content rank 单调下降；
- neighbor recovery 是否使用 `missing_chunks`，而不是固定重新要求 `K` 个 chunk；
- `B2` 是否能在同一场景下对 hot/cold request 做出不同选择；
- `B2` 是否使用 neighbor availability 和 cache probability 估计 expected delay；
