...
1. 原始三组 research-plan 场景中，B2 相对 B1 的总体延迟优势很小，不能夸大；
2. decision-boundary 场景和 fallback-stage 图显示，在 local recovery 已经失败、B1 的 neighbor-first 行为容易浪费探测延迟时，B2 的差异会变得明显；
3. hot/mid/cold rank bucket 图显示 B2 主要对热门内容选择 neighbor，对 mid/cold 内容抑制 neighbor 搜索。

### 1.1 用 nature-writing 梳理出的汇报主线

按照 `nature-writing` 的写作流程，这份报告的核心论证可以压缩成一句话：

> 在低可信 edge cache fallback 问题中，本阶段用 Monte Carlo simulation 证明 request-level expected-delay B2 可以根据请求内容热度和缺失 chunk 数选择是否搜索 neighbor ES；三组 research-plan 场景、一个 decision-boundary 诊断场景、fallback-stage analysis、neighbor/origin heatmap、Zipf/cache heatmap 和 rank-bucket analysis 共同支持这一点，但结论仍限于第一阶段概率模型。

为了让第一次接触这个项目的人能读懂，正文采用下面的阅读路线：

| 阅读步骤 | 读者需要先理解什么 | 本报告对应位置 |
| --- | --- | --- |
| 1 | B0/B1/B2 分别是什么 | 第 2 节 |
| 2 | 为什么旧 B2 不够明显 | 第 3 节 |
| 3 | 新 B2 如何逐请求判断 | 第 3 节公式 |
| 4 | 实验怎么跑、参数是什么 | 第 4-5 节 |
| 5 | 原始场景总体延迟为什么不够明显 | 第 6-7 节 |
| 6 | B2 在哪些参数区域更明显 | 第 8-9 节 |
| 7 | B2 是否真的对 hot/cold 请求做不同选择 | 第 10 节 |
| 8 | 哪些结论被证据支持、哪些还只是限制 | 第 11-15 节 |
...
本批次包含三组 research-plan 场景和一组 decision-boundary 诊断场景。诊断场景不是为了替代研究计划书的三组场景，而是为了检查 B2/B1 总体延迟差异不明显时，差异是否集中在 fallback 决策阶段。
...
| `decision_boundary_neighbor` | 0.20 | 80 ms | 诊断 B2 是否能在低价值 neighbor probing 区域明显区别于 B1。 |
...
## 6. Research-Plan 场景与 Decision-Boundary 诊断场景

下面三张图比较 B0、B1、B2 的 mean response time、p95 response time，以及只统计 local recovery 失败请求的 fallback-stage mean response time。前三个场景对应研究计划书中的主要实验：steady、low-reliability neighbor、origin-delay increase。第四个 `decision_boundary_neighbor` 是本次新增的诊断场景，用来专门回答一个问题：如果 B2 和 B1 在总体延迟上太接近，是否是因为 B2 几乎总是选择 neighbor，或者因为 local success 请求稀释了 fallback 决策差异？
...
**图 1. Mean response time。** 你的观察是对的：在原始三组 research-plan 场景里，B2 和 B1 的总体平均延迟差异并不明显。steady 场景中 B2 只快 0.2746 ms，low-reliability neighbor 场景快 1.5561 ms，origin-delay increase 场景只快 0.0769 ms。原因是 B2 在 steady 和 origin-delay increase 中几乎总是选择 neighbor，所以行为接近 B1；同时 local recovery 成功的请求也会稀释 fallback 决策差异。新增的 decision-boundary 场景中，B2 相对 B1 的总体均值优势为 6.5971 ms，这才是延迟差异更清楚的区域。

| 场景 | 策略 | mean | p95 | fallback mean | fallback p95 | neighbor attempt | B2 skip | B2 choice | B2 mean adv. | B2 fallback adv. |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| steady | B0 | 100.7794 | 201.4137 | 197.0367 | 203.9427 | 0.0000 | 1.0000 | 0.0000 |  |  |
| steady | B1 | 67.8475 | 225.9995 | 123.3427 | 228.7295 | 0.4469 | 0.0000 | 0.0000 |  |  |
| steady | B2 | 67.5729 | 225.1959 | 122.7274 | 227.9298 | 0.4081 | 0.0869 | 0.9131 | 0.2746 ms | 0.6153 ms |
| low_reliability_neighbor | B0 | 100.8571 | 201.4713 | 197.0226 | 203.9906 | 0.0000 | 1.0000 | 0.0000 |  |  |
| low_reliability_neighbor | B1 | 92.1258 | 228.3606 | 177.5018 | 230.8938 | 0.4474 | 0.0000 | 0.0000 |  |  |
| low_reliability_neighbor | B2 | 90.5697 | 227.4472 | 174.0249 | 230.1282 | 0.3708 | 0.1713 | 0.8287 | 1.5561 ms | 3.4769 ms |
| origin_congestion | B0 | 163.8599 | 341.4185 | 337.0190 | 343.9011 | 0.0000 | 1.0000 | 0.0000 |  |  |
| origin_congestion | B1 | 90.6103 | 365.9864 | 173.7167 | 368.6900 | 0.4486 | 0.0000 | 0.0000 |  |  |
| origin_congestion | B2 | 90.5334 | 365.9026 | 173.5448 | 368.6143 | 0.4450 | 0.0080 | 0.9920 | 0.0769 ms | 0.1719 ms |
| decision_boundary_neighbor | B0 | 56.1642 | 101.4799 | 97.0254 | 104.0185 | 0.0000 | 1.0000 | 0.0000 |  |  |
| decision_boundary_neighbor | B1 | 62.0956 | 128.5371 | 110.2674 | 131.1153 | 0.4480 | 0.0000 | 0.0000 |  |  |
| decision_boundary_neighbor | B2 | 55.4985 | 103.2879 | 95.5396 | 121.5895 | 0.0699 | 0.8441 | 0.1560 | 6.5971 ms | 14.7278 ms |

![P95 response time by scenario and policy](../results/phase1_b2_zipf/figures/fig_phase1_b2zipf_scenario_p95_response_time.svg)

**图 2. 95th percentile response time。** B2 相对 B1 的 p95 在原始三组 research-plan 场景里也只是轻微改善。decision-boundary 场景更清楚：B1 的 p95 为 128.5371 ms，B2 降到 103.2879 ms，因为 B2 大量跳过低价值 neighbor search。但是 B1/B2 的 p95 不一定优于 B0，因为 neighbor 搜索失败时，请求会先付出 neighbor probe delay，随后仍然访问 origin，形成更长尾延迟。

![Fallback-stage mean response time by scenario and policy](../results/phase1_b2_zipf/figures/fig_phase1_b2zipf_fallback_mean_response_time.svg)

**图 3. Fallback-stage mean response time。** 这张图只统计 `missing_chunks > 0` 的请求，也就是 local ES 已经无法恢复、必须进入 fallback 决策的请求。它比总体 mean 更适合评价 B1/B2 的决策差异。decision-boundary 场景下，B1 的 fallback mean 为 110.2674 ms，B2 为 95.5396 ms，差异为 14.7278 ms。这个结果说明：B2 的延迟优势不是在所有请求上平均铺开，而是在 local failure 后避免 B1 的低价值 neighbor probing。
...
**图 4. 低可靠 neighbor 场景下的 neighbor attempt / failure / origin-free rate。** 这张图解释了为什么 B2 在低可靠 neighbor 场景中优于 B1。B1 在每次 local failure 后都尝试 neighbor，所以 neighbor attempt rate 高，neighbor failure rate 也高。B2 会跳过一部分期望延迟不划算的 neighbor 搜索。
...
**图 5. B2 相对 B1 的优势随 neighbor availability 和 origin delay 的变化。** 颜色表示 `B1 mean response time - B2 mean response time`。数值为正表示 B2 比 B1 快。
...
**图 6. B2 相对 B1 的优势随 Zipf alpha 和 cache-rank gamma 的变化。** 这张图测试内容请求分布和 neighbor cache rank 敏感性是否会影响 B2 的可见性。
...
**图 7. B2 在不同内容热度 bucket 下的 neighbor choice rate。** 这是解释 Phase 1.1 最关键的一张图。它显示 B2 不再是场景级固定策略，而是会根据请求内容 rank 做不同选择。在这个 decision-boundary 设置下，B2 对 hot content 经常选择 neighbor，但对 mid/cold content 基本不选择 neighbor。
...
因此，如果想向老师或审稿人解释“B2 为什么有意义”，应该优先展示图 7 和图 3，然后再用图 5 和图 6 说明参数区域。

## 11. Claim-Evidence Map：哪些说法被结果支持

这一节是按照 `nature-writing` 的 reviewer-facing 写作要求加入的自审计：每个主要说法都必须对应具体证据，不能只凭直觉描述。

| 主要 claim | 对应证据 | 支持状态 | 写作时应如何表述 |
| --- | --- | --- | --- |
| B2 已经从 scenario-level switch 改成 request-level decision。 | `simulator.py` 记录 `content_rank`、`missing_chunks`、`b2_expected_neighbor_delay`、`b2_neighbor_selected`；第 3 节公式说明逐请求判断。 | supported | 可以明确说“本阶段实现了 request-level B2”。 |
| B2 在三组 research-plan 场景中相对 B1 有更低 mean response time。 | 图 1 和第 6 节表格：steady 优势 0.2746 ms，low-reliability 优势 1.5561 ms，origin-delay increase 优势 0.0769 ms。 | supported but weak | 必须写成“总体延迟差异很小”，不能写成“明显提升”。 |
| B2 在 decision-boundary 场景中能明显降低 B1 的低价值 fallback 延迟。 | 图 1-3 和第 6 节表格：总体 mean 优势 6.5971 ms；fallback-stage mean 优势 14.7278 ms；B2 skip rate 0.8441。 | supported in diagnostic setting | 可以说“当 B1 的 neighbor-first 处于决策边界或低价值探测区域时，B2 的延迟差异明显”。 |
| B2 可以降低 B1 在低可靠 neighbor 场景中的无效搜索。 | 图 4：neighbor attempt rate 从 B1 的 0.4474 降到 B2 的 0.3708；neighbor failure rate 从 0.7030 降到 0.6515。 | supported | 可以说“抑制一部分低价值 neighbor probing”。 |
| B2 的优势依赖参数区域。 | 图 5 和图 6：neighbor/origin heatmap 最大优势 6.6260 ms；Zipf/cache heatmap 最大优势 3.9040 ms。 | supported | 应强调“条件性优势”，不要说“所有场景都明显优于 B1”。 |
| B2 会根据内容冷热做不同选择。 | 图 7：hot bucket 的 B2 choice rate 为 0.6823；mid/cold bucket 为 0.0000。 | strongly supported in selected setting | 可以作为本报告最核心图示结论。 |
| 当前模型可以代表真实 CDN 拥塞。 | 当前只改变 origin delay，没有 arrival process、queue、service capacity。 | not supported | 不能这样写；只能说“origin-delay increase scenario”。 |
| 当前 binomial 模型已经完全真实地描述 neighbor cache。 | `p_cache(rank)` 是人工参数化模型，未接入真实 trace 或 cache replacement。 | not supported | 必须作为 limitation，并提出后续完善方向。 |

## 12. 写作自审计：第一次接触者能否看懂、这个实验是否有意义

### 12.1 第一次接触者可读性审计

| 审计问题 | 当前处理 | 结论 |
| --- | --- | --- |
| 读者是否先知道 B0/B1/B2 是什么？ | 第 2 节先给策略定义表，再解释 B1 的问题和 B2 的目的。 | pass |
| 读者是否知道为什么要改 B2？ | 第 1 节和第 3 节说明旧 B2 不够明显，新 B2 改成 request-level expected-delay decision。 | pass |
| 公式是否太突然？ | 公式前先解释字段含义，公式后解释三个新 cache 参数。 | pass |
| 实验是否可复现？ | 第 4-5 节列出 trials、requests、场景参数和命令；`manifest.json` 记录批次信息。 | pass |
| 每张图是否回答一个问题？ | 图 1/2 回答总体性能；图 3 回答 fallback-stage 差异；图 4 回答低可靠 neighbor；图 5/6 回答参数敏感性；图 7 回答 request-level 选择行为。 | pass |
| 是否说清楚结果不是万能提升？ | 第 6、8、11、13 节都限制了 claim，特别说明 p95 和真实拥塞模型问题。 | pass |
| 是否存在读者可能卡住的术语？ | `ES`、`origin`、`neighbor`、`Zipf`、`chunk` 仍带有专业性，但正文已在上下文中解释。 | minor risk |

可读性结论：这份报告已经可以让第一次接触项目的人理解“研究对象是什么、为什么需要 B2、B2 怎么判断、实验图说明了什么”。仍然可以在以后加入一张系统流程示意图，进一步降低阅读门槛。

### 12.2 研究意义审计

| 审计问题 | 当前证据 | 判断 |
| --- | --- | --- |
| 这个问题是否真实存在？ | 视频内容增大、origin 访问导致核心网络压力和延迟，是 CDN/edge cache 的基本动机。 | meaningful |
| B2 是否只是把 B1 包装了一下？ | 图 7 显示 B2 对 hot/mid/cold 请求做出不同选择；它不是固定 neighbor-first。 | meaningful |
| B2 的收益是否足够明显？ | 原始三组 research-plan 场景总体延迟收益很小；fallback-stage、decision-boundary、heatmap 和 rank bucket 更清楚。 | conditionally meaningful |
| 这是否足以作为完整论文结论？ | 还缺真实 cache placement、参数依据、queueing/congestion model。 | not yet |
| 下一步是否清楚？ | 需要文献/trace 支撑参数，完善 binomial 模型，再做 queueing 或 cache replacement 扩展。 | clear next step |

意义结论：本阶段结果有研究价值，但价值主要在“明确 B2 的条件性决策机制”，不是在“证明 B2 在真实 CDN 中全面优越”。这对研究计划书很重要，因为它把目标从过强的性能承诺，调整为更可靠的 policy-design evidence。

### 12.3 反复审计后的写作边界

为了避免过度宣称，后续汇报中建议使用以下表述。

| 不建议写 | 建议写 |
| --- | --- |
| B2 显著降低所有场景延迟。 | B2 在三组 research-plan 场景中的总体延迟优势很小；在 decision-boundary 和 fallback-stage 分析中差异更明显。 |
| B2 解决了 origin congestion。 | 本阶段只模拟 origin delay increase，尚未模拟真实排队拥塞。 |
| neighbor cache 模型已经真实。 | 当前使用 rank-aware synthetic cache probability，后续需要文献、trace 或 cache placement 模型支撑。 |
| B2 一定比 B1 好。 | B2 的价值在于根据 expected delay 选择是否搜索 neighbor，优势取决于 neighbor availability、origin delay 和内容热度分布。 |
...
| research-plan/diagnostic 场景实验 | 10 trials，每个 trial 10,000 requests |
| Zipf/cache sweep | 10 trials，每个 trial 10,000 requests |
| 图表生成 | 7 张 SVG/PDF 已生成；PNG/TIFF 仅用于本地视觉 QA |
...
| fallback-stage metrics | 只统计 `missing_chunks > 0` 的请求，避免被 local success 请求稀释。 |
...
| `src/edge_cache_sim/metrics.py` | 新增 summary metrics，例如 fallback-stage latency、neighbor attempt rate、B2 choice rate 和 B2 skip rate。 |
| `src/edge_cache_sim/repeated.py` | repeated-trial 聚合逻辑。 |
| `scripts/run_scenarios.py` | research-plan 场景和 decision-boundary 诊断场景实验入口。 |
...
Phase 1.1 的核心成果是让 B2 的行为变得可解释。你指出的“B2 和 B1 在延迟上基本没有差别”是正确的，至少在原始三组 research-plan 场景的总体 mean/p95 图上确实如此。因此本报告现在把结论收紧为：B2 不是在所有场景全面赢过 B1，而是在 local failure 之后、neighbor search 的价值处于边界或偏低时，能够跳过 B1 的低价值探测。

真正有说服力的是：

1. fallback-stage 图显示 B2 在 decision-boundary 场景中相对 B1 降低 14.7278 ms；
2. heatmap 显示 B2 在特定 neighbor/origin 和 Zipf/cache 参数区域明显优于 B1；
3. rank bucket 图显示 B2 会对热门内容选择 neighbor，对 mid/cold 内容抑制 neighbor。
