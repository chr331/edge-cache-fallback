...
> 在低可信 edge cache fallback 问题中，本阶段用 Monte Carlo simulation 证明 request-level expected-delay B2 可以根据请求内容热度和缺失 chunk 数选择是否搜索 neighbor ES；三场景对比、neighbor/origin heatmap、Zipf/cache heatmap 和 rank-bucket analysis 共同支持这一点，但结论仍限于第一阶段概率模型。
...
| B2 在三场景中相对 B1 有更低 mean response time。 | 图 1 和第 6 节表格：steady 优势 0.2746 ms，low-reliability 优势 1.5561 ms，origin-delay increase 优势 0.0769 ms。 | supported but modest | 应写成“小幅改善”，不能写成“大幅提升”。 |
| B2 可以降低 B1 在低可靠 neighbor 场景中的无效搜索。 | 图 3：neighbor attempt rate 从 B1 的 0.4474 降到 B2 的 0.3708；neighbor failure rate 从 0.7030 降到 0.6515。 | supported | 可以说“抑制一部分低价值 neighbor probing”。 |
| B2 的优势依赖参数区域。 | 图 4 和图 5：neighbor/origin heatmap 最大优势 6.6260 ms；Zipf/cache heatmap 最大优势 3.9040 ms。 | supported | 应强调“条件性优势”，不要说“所有场景都明显优于 B1”。 |
| B2 会根据内容冷热做不同选择。 | 图 6：hot bucket 的 B2 choice rate 为 0.6823；mid/cold bucket 为 0.0000。 | strongly supported in selected setting | 可以作为本报告最核心图示结论。 |
...
| 每张图是否回答一个问题？ | 图 1/2 回答总体性能；图 3 回答低可靠 neighbor；图 4/5 回答参数敏感性；图 6 回答 request-level 选择行为。 | pass |
...
| B2 是否只是把 B1 包装了一下？ | 图 6 显示 B2 对 hot/mid/cold 请求做出不同选择；它不是固定 neighbor-first。 | meaningful |
| B2 的收益是否足够明显？ | 三场景收益 modest；heatmap 和 rank bucket 更清楚。 | conditionally meaningful |
...
| 不建议写 | 建议写 |
| --- | --- |
| B2 显著降低所有场景延迟。 | B2 在当前 Monte Carlo 设置下相对 B1 有小幅平均延迟改善，并在部分参数区域优势更明显。 |
