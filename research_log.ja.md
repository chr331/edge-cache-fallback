...
## 2026-06-03：Phase 1.1 project 整理、request-level B2、Zipf sensitivity analysis

### 本段階のテーマ

この段階では，repository を今後も更新しやすい構成に整理し，前段階で課題になっていた「B2 の差が見えにくいこと」と「Zipf parameter の sensitivity analysis が不足していること」に対応しました。目的は，B2 を最終的な最適 policy と主張することではなく，B2 の decision を scenario-level の固定判断から request-level の判断へ変更し，content rank，local で不足している chunk 数，neighbor success probability に基づいて動作を変えられるようにすることです。

### 実施内容

- `docs/project_map.md` を追加し，README，docs，scripts，src，tests，results，memo，overleaf の役割を整理しました。
- `docs/experiment_guide.md` を追加し，Phase 1.1 の command，output files，metrics の意味を初心者向けに説明しました。
- `CHANGELOG.md` と `scripts/write_manifest.py` を追加し，各 result batch の更新日，実行 command，parameter，git state，output file list を記録できるようにしました。
- 新しい結果は `results/phase1_b2_zipf/` にまとめ，既存 result structure，memo，Overleaf link を壊さないようにしました。
- B2 を request-level decision に変更しました。`missing_chunks = K - local_chunks` を用い，content rank から neighbor cache probability を推定します。
- `neighbor_cache_hot_prob = 0.90`，`neighbor_cache_cold_prob = 0.15`，`neighbor_cache_rank_gamma = 1.0` を default parameter として追加しました。
- `scripts/run_b2_zipf_sweep.py` を追加し，`neighbor_es_availability x origin_delay` と `zipf_alpha x neighbor_cache_rank_gamma` の二つの sensitivity analysis を行えるようにしました。
- Phase 1.1 figures を再作成しました。versioned artifact は SVG/PDF を中心とし，PNG/TIFF は local QA preview として扱います。

### 正式結果の判断

正式 batch では `trials = 10`，`num_requests = 10000` を用いました。三つの scenario における B2 の B1 に対する mean response time advantage は，`steady = 0.2746 ms`，`low_reliability_neighbor = 1.5561 ms`，`origin_congestion = 0.0769 ms` でした。したがって，三 scenario の bar chart だけでは B2 が大きく優位であるとは言えません。

より重要な証拠は sensitivity analysis と rank-bucket figure です。`neighbor_es_availability x origin_delay` heatmap では，最大 B2 advantage は `6.626 ms` で，`neighbor_es_availability = 0.20`，`origin_delay = 80 ms` の条件で観察されました。これは，B1 の固定的な neighbor search が余分な遅延になりやすい領域で，B2 の抑制効果が明確になることを示しています。

`zipf_alpha x neighbor_cache_rank_gamma` heatmap では，`neighbor_cache_rank_gamma` が大きいほど B2 advantage が明確になり，最大値は `3.904 ms` でした。rank-bucket figure では，hot content に対する B2 neighbor choice rate は約 `0.6823` である一方，mid / cold bucket では `0.0` でした。これは，Phase 1.1 の B2 が常に neighbor search を行う policy ではなく，hot content を中心に neighbor search を選択していることを直接示しています。
...
現在の model はまだ first-stage Monte Carlo simulation であり，request arrival，queueing delay，service capacity，real CDN trace，online trust learning は含んでいません。B2 の probability model は前段階より説明しやすくなりましたが，まだ manually defined rank-aware probability model です。次の段階では，full discrete-event simulation に進む前に，historical observation に基づく Beta-Binomial / Bayesian update，または cache placement と content popularity を明示的に結び付ける拡張を検討できます。
