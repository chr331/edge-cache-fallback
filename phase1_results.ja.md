...
第一段階は、単一の baseline から、研究計画書に対応する三つの正式 scenario へ拡張されました。現在の出力は、`steady`、`low_reliability_neighbor`、`origin_congestion` の三つを含みます。
...
| `origin_congestion` | `0.82` | `0.82` | `320.0` | オリジン輻輳シナリオ |
...
`origin_congestion` scenario では、origin delay を `320.0` に上げています。この場合、直接 origin に戻る `B0` は大きく悪化し、mean response time は `163.6239`、p95 response time は `341.3924` になります。`B1` と `B2` は信頼できる neighbor ES を使えるため、`B0` より明らかに良い結果です。ただし neighbor が信頼できるため、`B2` も多くの場合 neighbor fallback を選び、`B1` との差は小さくなります。

## Sweep と heatmap

三つの scenario に加えて、第一段階では sensitivity / grid analysis も残しています。

```text
origin_delay x es_availability
```

36 個の grid parameter points のうち、以下の結果でした。

- `20` 点では `B2 advantage vs B1` が正であり、`B2` の平均応答時間が `B1` より低い結果でした。
- `16` 点では負でしたが、負の幅は小さいものでした。
- 最大 advantage は `origin_delay = 40.0`、`es_availability = 0.45` のときで、`18.0856` でした。
- 最小値は `origin_delay = 320.0`、`es_availability = 0.75` のときで、`-0.1796` でした。

この heatmap は、`B2` が常に `B1` に大きく勝つことを示すものではありません。むしろ、neighbor が低信頼で、origin cost が相対的に高すぎない領域で、`B2` が無駄な neighbor fallback を避けられることを示しています。

## Figure files

Figures は `scripts/build_figures.py` により Python + Matplotlib で生成し、`results/figures/` に出力しています。現在は 8 組の figure があります。
...
また、`origin_congestion` は現時点では `origin_delay` を大きくすることで表現しています。実際の congestion queue model ではありません。次段階で正式な discrete-event simulation に進む場合、request arrival、service rate、queueing delay、cache replacement policy などを加える必要があります。
